import { useForm } from "react-hook-form";
import { useState } from "react";
import { toast } from "react-toastify";

export default function FarmerForm() {
  const [location, setLocation] = useState("");

  const {
    register,
    handleSubmit,
    formState: { errors },
    setValue,
    reset,
  } = useForm();

  const onSubmit = async (data) => {
    // Transform data into the expected format
    console.log(data);
    const transformedData = {
      name: data.name,
      features: [
        {
          properties: {
            Name: data.nameBusiness,
            ID_Cliente: data.ID_Cliente,
            COORDENADA_X: data.COORDENADA_X,
            COORDENADA_Y: data.COORDENADA_Y,
          },
        },
      ],
    };

    try {
      const response = await fetch("api/farmer", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(transformedData),
      });
      if (response.ok) {
        console.log("Data submitted successfully:", transformedData);
        toast.success("Datos guardados con éxito");
        reset();
        setLocation("");
      } else {
        console.error("Error submitting data:", response.statusText);
        toast.warn("Error al enviar los datos: " + response.statusText);
      }
    } catch (error) {
      console.error("Error submitting data:", error);
      toast.error(`Error al enviar los datos: ${error.message || error}`);
    }
  };

  const getLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          const { latitude, longitude } = position.coords;
          const newLocation = `Lat: ${latitude}, Lon: ${longitude}`;
          setLocation(newLocation);
          setValue("ubicacion", newLocation);
        },
        (error) => {
          console.error("Error obteniendo la ubicación: ", error);
        }
      );
    } else {
      alert("Geolocalización no es soportada por este navegador.");
    }
  };

  return (
    <form
      className="p-2 w-full max-w-md mx-auto"
      onSubmit={handleSubmit(onSubmit)}
    >
      <input
        className="w-full p-3 mb-4 border rounded text-center"
        placeholder="Nombre de la Ruta"
        {...register("name")}
      />
      <input
        className="w-full p-3 mb-4 border rounded text-center"
        placeholder="Nombre del Negocio"
        {...register("nameBusiness")}
      />
      <input
        className="w-full p-3 mb-4 border rounded text-center"
        placeholder="ID Cliente"
        type="text"
        {...register("ID_Cliente")}
      />
      <input
        className="w-full p-3 mb-4 border rounded text-center"
        placeholder="Coordenada X"
        type="number"
        step="any"
        {...register("COORDENADA_X")}
      />
      <input
        className="w-full p-3 mb-4 border rounded text-center"
        placeholder="Coordenada Y"
        type="number"
        step="any"
        {...register("COORDENADA_Y")}
      />

      <button
        type="button"
        className="w-full bg-spin-secondary-blue text-white py-3 rounded mb-4 hover:bg-blue-500"
        onClick={getLocation}
      >
        Obtener Ubicación
      </button>
      <input
        className="w-full p-3 mb-4 border rounded text-center"
        placeholder="Ubicación"
        value={location}
        readOnly
      />
      <button
        type="submit"
        className="w-full bg-orange-500 text-white py-3 rounded hover:bg-green-500"
      >
        Registrar
      </button>
    </form>
  );
}
